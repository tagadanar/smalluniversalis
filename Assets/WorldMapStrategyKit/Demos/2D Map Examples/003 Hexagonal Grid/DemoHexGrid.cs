...
namespace WorldMapStrategyKit {
    ...
    public class DemoHexGrid : MonoBehaviour {
        ...
        float cellsCount;
        ACTION_MODE mode;
        readonly List<int> selectedCells = new List<int>();
        ...
        void AnimateCells(int row, int col) {
            ...
            const int RADIUS = 6;
            int cellIndex = map.GetCellIndex(row, col);
            map.GetCellsWithinRadius(cellIndex, RADIUS, selectedCells);

            foreach(int otherCellIndex in selectedCells) {
                int distance = map.GetCellStepsDistance(cellIndex, otherCellIndex);
                switch (mode) {
                    case ACTION_MODE.Flash:
                        map.CellFlash(otherCellIndex, Color.red, distance * 0.25f);
                        break;
                    case ACTION_MODE.Blink:
                        map.CellBlink(otherCellIndex, Color.red, distance * 0.25f);
                        break;
                    default:
                        map.CellFadeOut(otherCellIndex, Color.red, distance * 0.25f);
                        break;
                }
            }
        }
